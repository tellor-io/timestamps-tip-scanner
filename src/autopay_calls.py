...
class AutopayCalls:
    ...
    def __init__(
        ...
        w3: Web3,
        autopay_address: ChecksumAddress
    ) -> None:
        self.w3 = w3
    ...
    async def get_timestamps_before(self) -> Any:
        ...
        calls = [
            Call(
                self.autopay_address,
                ["getDataBefore(bytes32,uint256)(bool,bytes,uint256)", bytes.fromhex(query_id[2:]),timestamp],
                [["disregard",None],["disregard",None],[(query_id, timestamp), None]],
            )
    ...
    async def get_past_tips(self) -> Any:
        ...
        calls = [
            Call(
                self.autopay_address,
                ["getPastTips(bytes32)((uint256,uint256)[])", bytes.fromhex(query_id[2:])],
                [[query_id, None]],
            )
            for query_id in self.query_ids
        ]
